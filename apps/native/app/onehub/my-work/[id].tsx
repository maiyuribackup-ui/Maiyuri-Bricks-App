...
import { useProcessForLead } from "@/hooks/use-process";
...
export default function WorkItemDetail() {
  ...
  // Process OS stages are mirrored into My Work; the stage panel is the real
  // execution surface for them (checklist, gates, handover actions).
  const isProcessItem = item?.source_module === "process_os";
  const processView = useProcessForLead(
    isProcessItem ? (item?.related_lead_id ?? undefined) : undefined,
  );
  ...
  const respIdByTemplateItem = new Map(
    (item.checklist_instance?.responses ?? []).map((r) => [
      r.template_item_id,
      r.id,
    ]),
  );
  ...
  const onCompleteSimple = () =>
    complete.mutate(
      { note: note.trim() || null },
      {
        onSuccess: () =>
          toast.success(
            item.requires_approval ? "Sent for approval" : "Completed",
          ),
        onError: (e) =>
  ...
  return (
    ...
    >
      ...
      {isProcessItem ? (
        <View className="mt-4">
          {processView.isLoading ? (
            <View className="rounded-2xl bg-white p-4">
              <Text className="text-sm text-slate-400">
                Loading process stage…
              </Text>
            </View>
          ) : processView.data?.data ? (
            <>
              <ProcessStagePanel view={processView.data.data} />
              <Touchable
                onPress={() =>
                  router.push(
                    `/onehub/processes/case/${processView.data?.data?.instance.id}` as never,
                  )
                }
                className="mt-3 flex-row items-center justify-center gap-1.5 rounded-xl py-3"
              >
                <Icon name="git-branch-outline" size={16} color="#f97316" />
            ...
            <View className="rounded-2xl bg-white p-4">
              <Text className="text-sm text-slate-400">
                {item.related_lead_id
                  ? "No active process case for this lead."
                  : "This process stage is not linked to a lead yet."}
              </Text>
            </View>
          )}
        </View>
                  ...
                  onPress={() =>
                    ...
                    returnWork.mutate(
                      { reason: returnReason.trim() },
                      {
                        ...
                        onError: (e) =>
                          toast.error(
                            e instanceof Error ? e.message : "Return failed",
                          ),
                      },
                    )
                  }
  ...
  return (
    <View className="mb-2 rounded-xl border border-slate-200 bg-white p-3">
      ...
      {ti.input_type === "status" ? (
        <View className="mt-2 flex-row gap-2">
          {STATUS_CHIPS.filter(
            (c) => c.key !== "not_applicable" || ti.allow_na,
          ).map((c) => {
