...
class FloorPlanSupabaseService {
   ...
   * Get a session by ID
   */
  async getSession(sessionId: string): Promise<FloorPlanResult<DbFloorPlanSession>> {
    try {
      const supabase = getSupabaseAdmin();
  ...
  async updateCollectedInputs(
    ...
    try {
      ...
      const currentInputs = (session?.collected_inputs as Partial<FloorPlanInputs>) || {};
   ...
   */
  async getMessages(sessionId: string): Promise<FloorPlanResult<ChatMessage[]>> {
    try {
      const supabase = getSupabaseAdmin();
      ...
      const messages: ChatMessage[] = (data || []).map((msg: DbFloorPlanMessage) => ({
        id: msg.id,
        role: msg.role,
        content: msg.content,
  ...
  async updateProgress(
    sessionId: string,
    progress: Omit<DbFloorPlanProgress, 'id' | 'session_id' | 'created_at' | 'updated_at'>
  ): Promise<FloorPlanResult<void>> {
    try {
      const supabase = getSupabaseAdmin();
   ...
   */
  async getProgress(sessionId: string): Promise<FloorPlanResult<DbFloorPlanProgress | null>> {
    try {
      const supabase = getSupabaseAdmin();
  ...
  async updateModification(
    ...
    try {
      ...
      if (updates.trade_offs !== undefined) updateData.trade_offs = updates.trade_offs;
      ...
      if (updates.after_image !== undefined) updateData.after_image = updates.after_image;
   ...
   */
  async getModifications(sessionId: string): Promise<FloorPlanResult<DbFloorPlanModification[]>> {
    try {
      const supabase = getSupabaseAdmin();
  ...
  > {
    try {
      const [sessionResult, messagesResult, progressResult, modificationsResult] =
        await Promise.all([
