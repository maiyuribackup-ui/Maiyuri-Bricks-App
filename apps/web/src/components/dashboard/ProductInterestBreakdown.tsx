...
export function ProductInterestBreakdown({
  ...
  const sortedProducts = [...products].sort((a, b) => b.inquiries - a.inquiries);
            ...
            >
              ...
              <div className="text-right">
                ...
                <p className="text-[10px] text-slate-500">
                  {product.converted} converted ({product.inquiries > 0 ? Math.round((product.converted / product.inquiries) * 100) : 0}%)
                </p>
              </div>
            </div>
          ))}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-slate-600 dark:text-slate-400">
                Most requested: <strong className="text-slate-900 dark:text-white">{sortedProducts[0]?.label}</strong>
              </span>
            </div>
            <span className="text-sm font-medium text-orange-600">
              {Math.round((sortedProducts[0]?.inquiries / totalInquiries) * 100)}% of inquiries
            </span>
          </div>
        </div>
      )}
...
export function getDefaultProductInterests(): ProductInterest[] {
  return [
    { product: 'red_brick', label: 'Red Clay Brick', inquiries: 0, converted: 0, avgQuantity: 0, trend: 'stable' },
    { product: 'fly_ash', label: 'Fly Ash Brick', inquiries: 0, converted: 0, avgQuantity: 0, trend: 'stable' },
    { product: 'cement_block', label: 'Cement Block', inquiries: 0, converted: 0, avgQuantity: 0, trend: 'stable' },
    { product: 'hollow_block', label: 'Hollow Block', inquiries: 0, converted: 0, avgQuantity: 0, trend: 'stable' },
    { product: 'paver', label: 'Paver Block', inquiries: 0, converted: 0, avgQuantity: 0, trend: 'stable' },
  ];
}
