...
async function analyzeLead(id: string): Promise<{ success: boolean; data?: { lead: Lead; analysis: AnalysisResult } }> {
          ...
          <div className="space-y-4">
            ...
            {lead.next_action && (
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-xs text-slate-500 dark:text-slate-400">Next Action</p>
                  <p className="text-sm font-medium text-slate-900 dark:text-white">
                    {lead.next_action}
                  </p>
                </div>
            ...
            {analysisResult && (
              <>
                ...
                >
                  ...
                  <ChevronIcon className={cn('h-4 w-4 transition-transform', showDetails && 'rotate-180')} />
                </button>

                {showDetails && (
                  <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                    {/* Score Factors */}
                    {analysisResult.factors && analysisResult.factors.length > 0 && (
                      <div>
                        ...
                        <div className="space-y-1">
                          {analysisResult.factors.map((factor, idx) => (
                            <div key={idx} className="flex items-center gap-2 text-sm">
                              ...
                              <span className="text-slate-600 dark:text-slate-300">{factor.factor}</span>
                            </div>
                          ))}
                        </div>
                      </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              <div className="text-center">
                <div className={cn(
                  'text-3xl font-bold',
                  predictionResult.probability >= 0.7 ? 'text-green-600 dark:text-green-400' :
                  predictionResult.probability >= 0.4 ? 'text-yellow-600 dark:text-yellow-400' :
                  'text-red-600 dark:text-red-400'
                )}>
                  {Math.round(predictionResult.probability * 100)}%
                </div>
                <p className="text-xs text-slate-500 dark:text-slate-400">Probability</p>
              </div>
              <div className="flex-1 h-2 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                <div
                  className={cn(
                    'h-full rounded-full transition-all duration-500',
                    predictionResult.probability >= 0.7 ? 'bg-green-500' :
                    predictionResult.probability >= 0.4 ? 'bg-yellow-500' :
                    'bg-red-500'
                  )}
