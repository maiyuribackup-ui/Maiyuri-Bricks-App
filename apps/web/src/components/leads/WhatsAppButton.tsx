"use client";

import { useState } from "react";
import { useMutation } from "@tanstack/react-query";
import { Button, Modal, Spinner } from "@maiyuri/ui";
import { toast } from "sonner";
import { buildWhatsAppUrl } from "@maiyuri/shared";

interface WhatsAppButtonProps {
  leadId: string;
  contactNumber: string;
  className?: string;
}

interface WhatsAppResponse {
  message: string;
  phoneNumber: string;
  whatsappUrl: string;
}

async function generateWhatsAppResponse(
  leadId: string,
): Promise<WhatsAppResponse> {
  const res = await fetch(`/api/leads/${leadId}/whatsapp-response`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  });
  if (!res.ok) throw new Error("Failed to generate response");
  const data = await res.json();
  return data.data;
}

export function WhatsAppButton({
  leadId,
  contactNumber,
  className,
}: WhatsAppButtonProps) {
  const [showPreview, setShowPreview] = useState(false);
  const [generatedResponse, setGeneratedResponse] =
    useState<WhatsAppResponse | null>(null);

  const mutation = useMutation({
    mutationFn: () => generateWhatsAppResponse(leadId),
    onSuccess: (data) => {
      setGeneratedResponse(data);
      setShowPreview(true);
    },
    onError: (error) => {
      toast.error(
        `Failed to generate message: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    },
  });

  const handleOpenWhatsApp = () => {
    if (generatedResponse?.whatsappUrl) {
      window.open(generatedResponse.whatsappUrl, "_blank");
      setShowPreview(false);
    }
  };

  const handleDirectWhatsApp = () => {
    // Use centralized phone normalization (adds country code if needed)
    window.open(buildWhatsAppUrl(contactNumber), "_blank");
  };

  return (
    <>
      <div className={`flex gap-2 ${className}`}>
        <Button
          variant="secondary"
          size="sm"
          onClick={() => mutation.mutate()}
          disabled={mutation.isPending}
          className="flex-1 bg-green-50 hover:bg-green-100 dark:bg-green-900/20 dark:hover:bg-green-900/30 text-green-700 dark:text-green-400 border-green-200 dark:border-green-800"
        >
          {mutation.isPending ? (
            <>
              <Spinner size="sm" className="mr-2" />
              Generating...
            </>
          ) : (
            <>
              <WhatsAppIcon className="h-4 w-4 mr-2" />
              AI Response
            </>
          )}
        </Button>
        <Button
          variant="secondary"
          size="sm"
          onClick={handleDirectWhatsApp}
          className="bg-green-50 hover:bg-green-100 dark:bg-green-900/20 dark:hover:bg-green-900/30 text-green-700 dark:text-green-400 border-green-200 dark:border-green-800"
          title="Open WhatsApp directly"
        >
          <WhatsAppIcon className="h-4 w-4" />
        </Button>
      </div>

      {/* Preview Modal */}
      <Modal
        isOpen={showPreview}
        onClose={() => setShowPreview(false)}
        title="WhatsApp Message Preview"
        size="md"
      >
        <div className="space-y-4">
          <div className="bg-green-50 dark:bg-green-900/20 rounded-lg p-4 border border-green-200 dark:border-green-800">
            <div className="flex items-center gap-2 mb-3">
              <WhatsAppIcon className="h-5 w-5 text-green-600" />
              <span className="text-sm font-medium text-green-700 dark:text-green-400">
                Message to {generatedResponse?.phoneNumber}
              </span>
            </div>
            <div className="bg-white dark:bg-slate-800 rounded-lg p-3 shadow-sm">
              <pre className="text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap font-sans">
                {generatedResponse?.message}
              </pre>
            </div>
          </div>

          <div className="text-xs text-slate-500 dark:text-slate-400">
            This message was generated based on the lead&apos;s status,
            requirements, and conversation history. You can edit it in WhatsApp
            before sending.
          </div>

          <div className="flex justify-end gap-3">
            <Button variant="secondary" onClick={() => setShowPreview(false)}>
              Cancel
            </Button>
            <Button
              onClick={handleOpenWhatsApp}
              className="bg-green-600 hover:bg-green-700 text-white"
            >
              <WhatsAppIcon className="h-4 w-4 mr-2" />
              Open in WhatsApp
            </Button>
          </div>
        </div>
      </Modal>
    </>
  );
}

function WhatsAppIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor">
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
    </svg>
  );
}
