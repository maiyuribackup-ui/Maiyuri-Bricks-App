...
test.describe('Odoo Quote Display', () => {
  ...
  test('should display Odoo quotes without runtime errors on leads page', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="lead-details"], h1, .lead-name, [class*="lead"]', { timeout: 30000 });
    ...
    expect(criticalErrors, 'Should have no toLocaleString or null reference errors').toEqual([]);

    // Take a screenshot for manual verification
    await page.screenshot({ path: 'test-results/odoo-quote-display.png', fullPage: true });
  });

  test('should display quote amounts correctly after sync', async ({ page }) => {
    const errors = await trackErrors(page);
    ...
    // Take screenshot
    await page.screenshot({ path: 'test-results/odoo-quote-amounts.png', fullPage: true });
  });
});
