...
// Set E2E_TEST_EMAIL, E2E_TEST_PASSWORD in .env.local for testing
const TEST_PASSWORD = process.env.E2E_TEST_PASSWORD || "";

if (!TEST_PASSWORD) {
  throw new Error(
    "E2E_TEST_PASSWORD environment variable is required for tests",
  );
}

const TEST_USERS = {
  founder: {
    email: process.env.E2E_TEST_FOUNDER_EMAIL || "ram@maiyuri.app",
    password: TEST_PASSWORD,
    name: "Ram Kumaran",
    role: "founder",
  },
  accountant: {
    email: process.env.E2E_TEST_ACCOUNTANT_EMAIL || "kavitha@maiyuri.app",
    password: TEST_PASSWORD,
    name: "Kavitha",
    role: "accountant",
  },
  engineer: {
    email: process.env.E2E_TEST_ENGINEER_EMAIL || "srinivasan@maiyuri.app",
    password: TEST_PASSWORD,
    name: "Srinivasan",
    role: "engineer",
  },
...
test.describe("Phase 4: Password Reset Flow", () => {
  ...
  test("TC005: Reset password page handles invalid/missing token", async ({
    page,
  }) => {
    ...
    await expect(
      page
        .getByText(/invalid|expired|request.*new/i)
        .first()
...
test.describe("Phase 3 & 7: Team Management & Invitations", () => {
  ...
  test("TC006: Team tab displays all team members", async ({ page }) => {
    ...
    await expect(page.getByText(/@maiyuri\.app/i).first()).toBeVisible();
  });
...
test.describe("Phase 3: Accept Invitation Flow", () => {
  test("TC011: Accept invite page loads with valid token format", async ({
    page,
  }) => {
    ...
    await expect(
      page
        .getByText(
          /accept.*invitation|join.*team|set.*password|invalid|expired/i,
        )
        .first(),
    ).toBeVisible({ timeout: 10000 });
  });

  test("TC012: Accept invite shows error for invalid token", async ({
    page,
  }) => {
...
test.describe("Phase 6: Security Hardening", () => {
  ...
  test("TC010: Rate limiting returns 429 after too many requests", async ({
    request,
  }) => {
