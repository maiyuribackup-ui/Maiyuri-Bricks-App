...
const RESIDENTIAL_QUESTIONS: QuestionConfig[] = [
  ...
  },
  {
    id: "setbacks",
    question:
      "What are the required setback distances from plot boundaries (in feet)?",
    type: "form",
    fields: ["north", "south", "east", "west"],
    helpText:
      "Setbacks are the minimum distances that must be maintained from plot boundaries. Common values: 3-10 feet depending on local regulations.",
  },
  {
    id: "bedrooms",
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (questionId === "surveyUpload" && typeof answer === "string") {
      const surveyResult = await planningService.processSurveyImage(
        sessionId,
        answer,
      );
      ...
      if (surveyResult.success && surveyResult.dimensions) {
        ...
        return success({
          status: "collecting",
          message:
            "I couldn't extract the dimensions automatically. Let me ask for them manually.",
          nextQuestion: {
            id: "plotDimensions",
            question: "Please enter your plot dimensions (in feet):",
            type: "form",
    ...
    await floorPlanSupabase.setCurrentQuestion(
      sessionId,
    ...
    if (prevQuestion?.options) {
      const selectedOption = prevQuestion.options.find(
        (o) =>
          o.value === answer ||
          (Array.isArray(answer) && answer.includes(o.value)),
      );
    ...
    return success({
      ...
      progress: {
        ...
        total: questions.filter(
          (q) => !q.condition || q.condition(updatedSession.inputs),
        ).length,
      },
    });
  } catch (err) {
