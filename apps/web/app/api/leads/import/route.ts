  ...
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const followUpDateRaw =
        row["Followup Due Date"] ?? row["followup due date"];

      // Get date fields from Excel (they come as serial numbers)
      const leadAddedDateRaw = row["Lead Added Date"] ?? row["lead added date"];
      const lastUpdatedDateRaw =
        row["Last Updated Date"] ?? row["last updated date"];

      // Get notes from Excel
      const staffNotes = findColumn(row, [
        "Notes",
        "notes",
        "comments",
        "remarks",
