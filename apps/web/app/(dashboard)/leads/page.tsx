...
type ViewType = 'all' | 'today' | 'follow_ups' | 'hot' | 'needs_attention' | 'archived';
...
async function fetchLeads(status: string, search: string, page: number, isArchived: boolean, fromDate?: string, toDate?: string) {
...
export default function LeadsPage() {
  ...
  const [sortBy, setSortBy] = useState<'created_at' | 'updated_at' | 'name' | 'ai_score'>('updated_at');
  ...
  const archiveMutation = useMutation({
    mutationFn: ({ id, archive }: { id: string; archive: boolean }) => updateLeadArchiveStatus(id, archive),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['leads'] });
  ...
  // Stats Logic (Note: This runs on fetched data which is paginated, ideally stats should be a separate API call or accurate only for current page if not handled backend)
  // For 'Today', 'Follow Ups' etc tabs, we filter client side from *fetched* or active view. 
  // However, since API filters 'is_archived', the counts of 'newToday' will be correct for the active view.
  // We'll keep the existing logic but be aware it only counts loaded leads or we should fetch global stats separate.
  // For simplicity MVP phase 1, we rely on loaded data plus current stats logic.
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      // ... keep existing calc for simplicity on current page
  ...
  const filteredLeads = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      ...
      if (sortBy === 'name') {
        ...
        valA = new Date(a.updated_at).getTime(); valB = new Date(b.updated_at).getTime();
      } else if (sortBy === 'ai_score') {
        valA = a.ai_score || 0; valB = b.ai_score || 0;
      }
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="hidden lg:grid lg:grid-cols-12 gap-4 px-6 py-3 bg-slate-50 dark:bg-slate-800/50 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider border-b border-slate-200 dark:border-slate-700 select-none">
                <div className="col-span-3 cursor-pointer hover:text-slate-800" onClick={() => toggleSort('name')}>
                  Lead {sortBy === 'name' && (sortOrder === 'asc' ? '↑' : '↓')}
                </div>
                <div className="col-span-2">Contact</div>
                <div className="col-span-2">Source / Type</div>
                <div className="col-span-1">Status</div>
                <div className="col-span-1 cursor-pointer hover:text-slate-800" onClick={() => toggleSort('ai_score')}>
                  Score {sortBy === 'ai_score' && (sortOrder === 'asc' ? '↑' : '↓')}
                </div>
                <div className="col-span-1 cursor-pointer hover:text-slate-800" onClick={() => toggleSort('created_at')}>
                  Created {sortBy === 'created_at' && (sortOrder === 'asc' ? '↑' : '↓')}
                </div>
                <div className="col-span-1 cursor-pointer hover:text-slate-800" onClick={() => toggleSort('updated_at')}>
...
function LeadRow({ lead, onHover, isHovered, onArchive, onWhatsApp, onCall }: {
  lead: Lead;
  ...
  return (
    ...
    >
      ...
      <div className="col-span-1 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
        <button onClick={onCall} title="Call" className="p-1 hover:bg-green-100 text-green-600 rounded">
          📞
        </button>
        <button onClick={onWhatsApp} title="WhatsApp" className="p-1 hover:bg-green-100 text-green-600 rounded">
          💬
        </button>
        <button onClick={onArchive} title={lead.is_archived ? "Unarchive" : "Archive"} className="p-1 hover:bg-slate-200 text-slate-600 rounded">
