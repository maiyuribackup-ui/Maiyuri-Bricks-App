...
export async function analyzeCall(
  ...
  try {
    ...
    if (!parseResult.success || !parseResult.data) {
      ...
      return {
        success: false,
        error: "Failed to parse transcript Q&A",
    ...
    for (const pair of qaPairs) {
      // ... (existing loop logic) ...
      const searchResult = await knowledgeCurator.searchKnowledge(
        pair.question,
        { limit: 3 },
      );
      const kbSources =
        searchResult.success && searchResult.data ? searchResult.data : [];
      ...
      if (evaluation.type === "coaching") {
        ...
        const { data: qData, error: qError } = await services.supabase.supabase
        ...
        if (qError) {
          console.error(
            "[SalesCoach] Error creating unanswered question:",
            qError,
          );
          continue;
        }
        ...
        if (qData) {
          const [leadResult, noteResult] = await Promise.all([
            services.supabase.getLead(leadId),
            services.supabase.getNote(noteId),
          ]);
          const assignedTo = noteResult.success
            ? noteResult.data?.staff_id
            : undefined;
          const fallbackAssignee = leadResult.success
            ? leadResult.data?.assigned_staff
            : undefined;
          const taskRes = await services.tasks.createTask({
            title: `Update KB: ${pair.question.substring(0, 80)}...`,
            description: `Missing info detected in sales call (Note ${noteId}).\nQuestion: ${pair.question}\nContext: ${pair.context}`,
            priority: "medium",
            dueDate: new Date(Date.now() + 86400000 * 2).toISOString(),
            leadId: leadId,
            assignedTo: assignedTo || fallbackAssignee || undefined,
          });
          ...
          if (taskRes.success && taskRes.data?.id) {
            await services.supabase.supabase
              .from("unanswered_questions")
              .update({ status: "task_created", task_id: taskRes.data.id })
              .eq("id", qData.id);
          }

          await services.telegram.notifyNewQuestion(
            pair.question,
            pair.context,
          );
        }
